...
namespace PhiJudge.Plugin.Language.Python
{
    internal class ExecutionStage : IExecutionStage
    {
        ...
        {
            _logger.LogInformation("Executing test point {0}+{1} in {2} from source file", new DirectoryInfo(directory).Name, testPoint.Order, directory);

            var profileFilePath = Path.Combine(directory, "profile.dat");
            var targetProgramPath = Path.Combine(directory, "source.py");
            var targetProcess = new Process
            {
                StartInfo = new()
                {
                    FileName = "/bin/busybox",
                    Arguments = $"time -v -o \"{profileFilePath}\" python3 \"{targetProgramPath}\"",
