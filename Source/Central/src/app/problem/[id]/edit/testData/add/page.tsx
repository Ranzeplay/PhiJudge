...
export default function Page({ params }: { params: { id: string } }) {
	...
	useEffect(() => {
		setAllowedToSubmit((timeLimit !== null && timeLimit !== 0) && (memoryLimit !== null && memoryLimit !== 0));
	}, [timeLimit, memoryLimit]);

	const router = useRouter();
	function submit() {
		HandleTestDataAddition(parseInt(params.id), timeLimit!, memoryLimit!, inputData, expectedOutputData)
	...
	return (
		<>
			...
			<div className="block space-x-2">
				<Button disabled={!allowedToSubmit} onClick={submit}>Submit</Button>
				<Button variant={'outline'} asChild>
					<Link href={'/problem/[id]/edit/testData'} as={`/problem/${params.id}/edit/testData`}>Cancel</Link>
				</Button>
			</div>
		</>
	)
